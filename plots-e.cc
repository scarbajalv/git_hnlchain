...
int main(){
  ...
  double brfactor431[15]={
    0.0000001981493085744838,  
    0.0000001782658582123428,
    0.0000001720547128667331,
    0.0000001545042688884745,
    0.000000141192921026764,
    0.0000001322130164526622,
    0.0000001049337689790622,
    0.00000007136782211589164,
    0.00000005497703611755734,
    0.000000041166074970886,
    0.00000003091608466888499,
    0.00000002350875842020808,
    0.00000001651722687670851,
    0.00000000846868310109085,
    0.000000001139596470631342};
    ...
    for(int l = 0; l < 41; ++l){
      for(int m = 0; m < 3; ++m){
        ...
        for (int i=0; i < nbins[m]; ++i){
            data << gsl_histogram_get( h[k][l][m], i)*brfactor431[k]<< " ";
        }
        data << endl;
      }
    }
    ...
    for(int l = 0; l < 41; ++l){
      for(int m = 0; m < 3; ++m){
        ...
        for (int i=0; i < nbinsbar[m]; ++i){
            databar << gsl_histogram_get( hbar[k][l][m], i)*brfactor431[k]<< " ";
        }       
        databar << endl;
      }
    }
